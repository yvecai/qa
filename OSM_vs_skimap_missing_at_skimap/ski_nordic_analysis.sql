...
----------------------------------------------------------------------------
\echo "... create osm_nordic_areas table";
    DROP TABLE IF EXISTS osm_nordic_areas;
    CREATE TABLE osm_nordic_areas ();
    SELECT AddGeometryColumn('osm_nordic_areas', 'geom', 900913, 'GEOMETRY', 2);
    ALTER TABLE osm_nordic_areas ADD COLUMN id SERIAL PRIMARY KEY;

----------------------------------------------------------------------------
\echo "... split big pistes buffers and add them to osm_nordic_areas";
...
-- 
    ...
    ALTER TABLE tmp ADD COLUMN id SERIAL PRIMARY KEY;
     ...
     -- ~ INSERT INTO osm_nordic_areas (geom)
    ...
    -- ~ CREATE INDEX osm_nordic_areas_idx
        -- ~ ON osm_nordic_areas USING gist(geom);
    -- ~ -- BOF
        -- ~ DELETE FROM osm_nordic_areas as big
        -- ~ USING tmp as small
        -- ~ WHERE 
...
----------------------------------------------------------------------------
\echo "... add smaller pistes buffers to osm_nordic_areas";
    INSERT INTO osm_nordic_areas (geom)
        SELECT geom
        FROM ubuffer
        WHERE
    ...
    -- ~  select count(*) from osm_nordic_areas;
     -- ~ count 
    -- ~ -------
      -- ~ 5880
    -- ~ (1 row)
...
-- compute smallest distance
    ALTER TABLE osm_nordic_areas ADD COLUMN distance float;

    UPDATE osm_nordic_areas
    SET distance = (
        select dist
        from (
            select 
            ...
            from union_skimaps as sk, osm_nordic_areas as osm
            ) sq
        where idx = id
        );
...
SELECT count(*) as closer_than_2km FROM osm_nordic_areas Where distance <2000;
\echo "Closer than 5km fromn a skimap ski area:";
SELECT count(*) as farther_than_2km FROM osm_nordic_areas Where distance < 5000;
\echo "Farther than 5km fromn a skimap ski area:";
SELECT count(*) as farther_than_5km FROM osm_nordic_areas Where distance > 5000;
